...
import WrongUserError from "./classes/wrong-user.error.js";
...
const errorThrower = {
  ...
  characterUserChecker: async function (user, where, select) {
    if (select && Object.keys(select).length > 0) select.userId = true;
    const query = queryBuilder(where, select);
    const character = await prisma.characters.findFirstOrThrow(query);
    if (character.userId != user.userId) throw new WrongUserError();
    return character;
  },
  characterMoneyChecker: async function (user, price, count, where, select) {
    if (select && Object.keys(select).length > 0) select.money = true;
    const character = await this.characterUserChecker(user, where, select);
    if (!price && !count) return character;
    if (price * count > character.money) throw new NotEnoughMoneyError("");
    return character;
  },
  ...
  itemSellChecker: async function (count, where, select) {
    ...
    const inventory = await prisma.inventories.findFirst(query);
    if (!inventory) throw new ItemNotFoundError("Item not in your inventory.");
